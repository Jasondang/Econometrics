...
```{r}
resettest(taylorModel, power = 2, type = "fitted", data = federalRatesData)
```
...
```{r}
summary(lm(INTRATE ~ INFL + COMMPRI + PCE + PERSINC + HOUST, data = federalRatesData))
```
...
federalRatesData$Year <- substr(federalRatesData$OBS, 1, 4)
grp <- federalRatesData[federalRatesData$Year < 1980, ]
x1 <- grp[, c("Year", "INFL", "PROD")]; 
y1 <- data.frame( INTRATE = grp["Year","INTRATE"] )
grp <- federalRatesData[federalRatesData$Year >= 1980, ]
x2 <- grp[, c"Year", "INFL", "PROD")]; y2 <- data.frame( INTRATE = grp["Year", "INTRATE"] )
